...
public class PostServiceImp implements PostService {
	...
	public boolean insertCommunity(String community) {
		...
		// 커뮤니티 정보가 null이 아니면 false 리턴
		if(communityVo != null) {
			return false;
		}
		
		// 다오에게 커뮤니티명을 주고 DB에 추가하고 시킨후 성공 여부를 리턴
		return postDao.insertCommunity(community);
	}

	@Override
	public boolean updateCommunity(String oldName, String newName) {
		// 이전 커뮤니티명과 이후 커뮤니티명 중에 null이 있거나 이후 커뮤니티명의 길이가 0이면 false 리턴
		if(oldName == null || newName == null || newName.trim().length() == 0) {
			return false;
		}
		// 이전 커뮤니티명과 이후 커뮤니티명이 같으면 false 리턴
		if(oldName.equals(newName)) {
			return false;
		}
		// 등록된 커뮤니티명이여야 수정할 수 있다.
		// 이전 커뮤니티명을 이용해서 커뮤니티VO를 가져옴
		CommunityVO oldVo = postDao.selectCommunity(oldName);

		// 커뮤니티VO가 null이면 false 리턴
		if(oldVo == null) {
			return false;
		}
		
		// 바꾸려는 커뮤니티명이 이미 등록되어 있으면 수정할 수 없다.
	...
	public boolean insertPost(PostVO post) {
		// 게시글VO null 체크해서 문제있으면 false 리턴
		if(post == null) {
			return false;
		}
		
		// 제목 null 체크, 빈문자열 체크해서 문제있으면 false 리턴
		if(!checkString(post.getPo_title())) {
			return false;
		}
		
		// 내용 null 체크, 빈문자열 체크해서 문제있으면 false 리턴
