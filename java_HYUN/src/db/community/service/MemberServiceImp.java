...
public class MemberServiceImp implements MemberService {
	...
	public MemberVO login(String id, String pw) {
		...
		// 회원 정보가 있으면 회원의 비번과 입력받은 비번을 확인해서 같은 회원정보를 반환
	...
	public boolean signup(String id, String pw, String email) {
		// 아이디 중복 검사
		// 다오에게 아이디를 주면서 회원 정보를 가져오라고 시켜서 있으면 false 리턴
		MemberVO user = memberDao.selectMember(id);
		if(user != null) {
			return false;
		}
		
		// 아이디가 정규표현식에 맞지 않으면 false 리턴
		String idRegex = "^\\w{6,13}$";
		if(!Pattern.matches(idRegex, id)) {
			return false;
		}
		
		// 비번이 정규표현식에 맞지 않으면 false 리턴
		...
		return memberDao.insetMember(id, pw, email);
	}
}
