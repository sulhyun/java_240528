...
public class phoneEx01 {
	...
	// 클래스(정적) 멤버 변수로 Scanner 를 만들면 어디에서든 매개변수로 넘기지 않고 사용가능하다.
	...
	public static void main(String[] args) {
		...
		// 반복문에 들어가면 조건식에서 사용하지 못하기 때문에 여기에 선언
		final int EXIT = 5;
		int menu = EXIT + 1;
		...
		} while(menu != EXIT /* 입력한 메뉴가 종료가 아닐때 */);
	}
	...
	/** 기능 : 연락처 리스트에 연락처가 몇번지에 있는지 알력주는 메소드
	 * @param list 연락처 리스트 
	 ...
	 * @param list 연락처 리스트
	 * @param count 저장된 개수
	 * @return 삭제 후 저장된 개수(삭제 성공이면 -1, 실패혐 그대로) */
...
class Contact {
	...
	/* toString 을 오버라이딩 하면 이름 : 번호 형태의 문자열이 필요할 때 사용하기가 좋다
	 * (여러번 필요하기 때문에) */
	...
	/* hashCode equals 는 전화번호가 같으면 동록이 되면 안되고, 수정할 때도 있는 번호는 
	 * 등록되면 안되기 때뭄에 equals 를 오버라이딩 하면 같은 번호인지 아닌지 비교하기가 쉽다. */
