...
SELECT MO_TITLE 영화, TH_NAME 영화관, SC_NAME 상영관,
	   SD_DATE 상영일, SD_TIME 상영시간
FROM 
	SCHEDULE
JOIN
...
# 위 예제를 서브 쿼리를 이용
SELECT 
    MO_TITLE 영화, TH_NAME 영화관, SC_NAME 상영관, 
    SD_DATE 상영일, SD_TIME 상영시간
FROM
    (SELECT * FROM SCHEDULE WHERE SD_DATE = '2024-07-27') SD
JOIN
	SCREEN ON SD_SC_NUM = SC_NUM
JOIN
...
SELECT TH_NAME 영화관, SC_NAME 상영관, SE_NAME 좌석, SC_SEAT 좌석수
FROM
	SCREEN
JOIN
...
WHERE SC_NAME = '1관' AND TH_NAME = 'CGV강남';

# abc123회원이 예약한 예매 내역을 조회하는 쿼리(영화 제목, 극장이름, 상영관 이름, 시간, 좌석번호)
SELECT MO_TITLE 영화, SD_TIME 시간, TH_NAME 극장, 
...
# 1번 스케줄에 예약된 좌석이 아닌 + 1번 상영관 좌석
SELECT SE_NAME
FROM
	SEAT
WHERE SE_SC_NUM = 1 AND SE_NAME NOT IN
(SELECT SE_NAME
FROM
	(SELECT * FROM TICKETING WHERE TI_SD_NUM = 1)TT
JOIN
...
SELECT COUNT(SE_NAME) '예약 가능한 좌석수'
FROM
	SEAT
WHERE SE_SC_NUM = 1 AND SE_NAME NOT IN
(SELECT SE_NAME
...
SELECT GE_NAME 장르, COUNT(MG_NUM) 영화수
FROM
	MOVIE_GENRE 
RIGHT JOIN
...
FROM
	MOVIE
WHERE MO_DATE BETWEEN DATE_SUB(NOW(), INTERVAL 1 MONTH) AND NOW();
