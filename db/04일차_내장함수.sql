...
# 탐색
# FIELD(찾을 문자열, 문자열1, 문자열2, ...) : 문자열들에서 찾을 문자열이 몇번째에 있는지 반환
# 1부터 시작. 없으면 0 (완전히 똑같아야 함)
...
# LPAD(문자열, 자리, 채울문자열) : 문자열이 자리수보다 작으면 왼쪽에 채울 문자열을 추가하여 자리수를 맞춤
# RPAD(문자열, 자리, 채울문자열) : 문자열이 자리수보다 작으면 오른쪽에 채울 문자열을 추가하여 자리수를 맞춤

# RGB(255,20,3)을 16진수로 변경해서 #XXXXXX으로 표현하는 쿼리
SELECT CONCAT('#', LPAD(HEX(255),2,'0'), LPAD(HEX(20),2,'0'), LPAD(HEX(3),2,'0')) 'RGB(255,20,3)';

# 문자열 교체
# INSERT(문자열, 위치, 길이, 대체문자열) : 문자열에서 위치부터 길이만큼 문자열을 제거 후 데체문자열을 추가
