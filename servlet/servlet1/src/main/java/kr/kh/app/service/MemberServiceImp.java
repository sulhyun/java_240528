...
public class MemberServiceImp implements MemberService {
	...
	public boolean signup(LoginDTO member) {
		...
		// 아이디 중복 검사해서 있으면 false 리턴
		if(memberDao.selectMember(member.getId()) != null) {
			return false;
		}
		// 유효성 검사 실패 시 false 리턴
	...
	private boolean checkValidate(LoginDTO member) {
		...
		// 이메일 유효성 검사.
		String emailRejex = "^[A-Za-z0-9_]+@[A-Za-z0-9_]+(\\.[A-Za-z]{2,}){1,}$";
		if(!Pattern.matches(emailRejex, member.getEmail())) {
			return false;
		}
		return true;
	}
