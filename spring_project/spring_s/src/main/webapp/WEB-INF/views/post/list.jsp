		...
		<c:forEach items="${list}" var="co">
			<c:choose>
				<c:when test="${co.co_num eq pm.cri.co_num }">
					<a href="<c:url value="/post/list?co_num=${co.co_num}"/>" class="btn btn-dark">${co.co_name}</a>
				</c:when>
				<c:otherwise>
					<a href="<c:url value="/post/list?co_num=${co.co_num}"/>" class="btn btn-outline-dark">${co.co_name}</a>
				</c:otherwise>
			</c:choose>
		...
		<form action="<c:url value="/post/list"/>">
			...
			<div class="input-group mb-3">
				...
				<div class="input-group-append">
					<button type="submit" class="btn btn-outline-dark col-12">검색</button>
				</div>
			</div>
		</form>
		<a href="<c:url value="/post/insert?co_num=${pm.cri.co_num}"/>" class="btn btn-outline-dark btn-insert">글등록</a>
	</c:if>
